...
function fixCrashlytics(fbNode: ts.Node, path: string): ReplaceChange|null {
    let eventNode = traversal.findSuccessor(fbNode, [ts.SyntaxKind.PropertyAccessExpression, ts.SyntaxKind.Identifier]);
...
function getTriggerType(sourceNode: ts.Node, fbFunctionsImportName: string): string {
...
function rewriteEvents(path: string): Change[] {
    ...
    for (let fbNode of firebaseFunctionNodes) {
        ...
        let eventBlockNode: ts.Node|null|undefined = traversal.findSuccessor(arrowFunctionNode, [
                ts.SyntaxKind.Block,
                ts.SyntaxKind.SyntaxList
            ]
        );
...
function fixDatabaseEvents(fbNode: ts.Node, nodeText: string, path: string, eventParamName: string, eventParamNameToWrite: string): string {
...
function fixFirestoreEvents(fbNode: ts.Node, nodeText: string, eventParamName: string, eventParamNameToWrite: string): string {
